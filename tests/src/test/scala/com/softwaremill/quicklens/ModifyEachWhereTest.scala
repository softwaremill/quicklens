...
class ModifyEachWhereTest extends FlatSpec with ShouldMatchers {
  ...
  it should "modify only those list elements where the condition returns true" in {
    modify(y1)(_.y2.y3.eachWhere(_.y4.map(_.name) == Some("d2")).y4.each.name).using(duplicate) should be (y1at1dup)
  }
